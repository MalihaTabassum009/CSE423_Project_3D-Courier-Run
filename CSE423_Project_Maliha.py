...
PLAYER_SPEED_SPRINT = 275.0  # Player sprint speed
...
time_left = 120.0       # The main countdown timer
total_score = 0
...
#-----------------------------------------------------------------------------------------
# --- 2D HUD begin/end helpers (so bars/quads draw in screen pixels) ---
def hud_begin():
    glMatrixMode(GL_PROJECTION); glPushMatrix(); glLoadIdentity()
    gluOrtho2D(0, WINDOW_WIDTH, 0, WINDOW_HEIGHT)   # screen-space (0..W, 0..H)
    glMatrixMode(GL_MODELVIEW);  glPushMatrix(); glLoadIdentity()

def hud_end():
    glPopMatrix()                                    # MODELVIEW
    glMatrixMode(GL_PROJECTION); glPopMatrix()       # PROJECTION
    glMatrixMode(GL_MODELVIEW)
#-----------------------------------------------------------------------------------------
...
### D5. HUD Drawing ###
def draw_hud():
    """Draws the Heads-Up Display with all game information (2D screen space)."""
    hud_begin()  # <-- enter orthographic HUD mode
    ...
    # Put it right under Score
    bar_left   = 10
    bar_right  = 210
    bar_top    = WINDOW_HEIGHT - 100     # <- move near the top-left
    bar_bottom = WINDOW_HEIGHT - 120

    # Label
    draw_text(bar_left, bar_top + 10, "Stamina")  # small offset above bar

    # (Optional but robust): turn off texturing/lighting while drawing HUD quads
...
#----------------------------------------------------------------------------
    # >>> REPLACE your current stamina-foreground block with THIS <<<
    # Foreground width proportional to stamina
    fill_w = (bar_right - bar_left) * (stamina / STAMINA_MAX)
    # Clamp just in case
    fill_w = max(0.0, min(bar_right - bar_left, fill_w))

    # Solid green if you have stamina, gray if empty
    if stamina > 0.0:
    ...
    glBegin(GL_QUADS)
    glVertex2f(bar_left,          bar_bottom)
    glVertex2f(bar_left + fill_w, bar_bottom)
    glVertex2f(bar_left + fill_w, bar_top)
    glVertex2f(bar_left,          bar_top)
    glEnd()
    # <<< END replacement >>>
...
#----------------------------------------------------------------------------
    ...
    # Package status
    status = "Empty"; color = COLOR_WHITE
    if is_carrying_package:
        if carried_package_info['is_correct']:
            status, color = "Correct Package", COLOR_GREEN
        else:
            status, color = "WRONG PACKAGE!", COLOR_RED
    ...
    hud_end()    # <-- leave orthographic HUD mode
